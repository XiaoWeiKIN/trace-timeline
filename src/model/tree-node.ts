...
//
// 移植自 grafana TraceView/components/utils/TreeNode.ts（原样）。
...
export default class TreeNode<TValue> {
  ...
  static iterFunction<TValue>(
    fn: ((value: TValue, node: TreeNode<TValue>, depth: number) => TreeNode<TValue> | null) | AnyFn,
    depth = 0
  ) {
    return (node: TreeNode<TValue>) => fn(node.value, node, depth);
  }

  static searchFunction<TValue>(search: AnyFn | TreeNode<TValue>) {
  ...
  find(search: AnyFn | TreeNode<TValue>): TreeNode<TValue> | null {
  ...
  getPath(search: AnyFn | TreeNode<TValue>) {
    const searchFn = TreeNode.iterFunction(TreeNode.searchFunction(search));
  ...
  walk(fn: (spanID: TValue, node: TreeNode<TValue>, depth: number) => void, startDepth = 0) {
    type StackEntry = { node: TreeNode<TValue>; depth: number };
  ...
  paths(fn: (pathIds: TValue[]) => void) {
    type StackEntry = { node: TreeNode<TValue>; childIndex: number };
