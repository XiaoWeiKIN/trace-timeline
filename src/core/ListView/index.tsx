...
export default class ListView extends React.Component<TListViewProps> {
  ...
  render() {
    ...
    // 适配（standalone）：当调用方提供了外部 scrollElement（非 windowScroller）时，
    // 该外部元素就是唯一滚动容器（componentDidMount 把 `_wrapperElm = scrollElement`，
    // scrollToIndex 也滚它）；此处内层 wrapper 不能再自带 overflow，否则会抢走滚动、
    // 使 _wrapperElm.scrollTop 恒为 0 且 scrollToIndex 失效。仅当无外部 scrollElement
    // （wrapper 自身需充当滚动容器）时才加 height:100%/overflow。
    if (!this.props.windowScroller && !this.props.scrollElement) {
    ...
    return (
      <div {...wrapperProps} data-testid="ListView">
        <div style={scrollerStyle}>
          <div
            style={{
              // width:100% 让绝对定位的 item-holder 撑满滚动容器，行的 width:100% 才能解析；
              // 上游靠注入的 itemsWrapperClassName(width:100%) 达成，这里内联等价实现（保 core 自含）。
              width: '100%',
              position: 'absolute',
              top: 0,
              margin: 0,
