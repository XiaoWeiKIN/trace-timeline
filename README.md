...
- **数据**：**可插拔后端适配器**（AD-15）——核心包后端中立，只认规范 `Trace`；适配器经子路径按需引入：
  - `@datafox/trace-timeline/adapters/otlp` —— OpenTelemetry OTLP/JSON（公开参考实现）
  - `@datafox/trace-timeline/adapters/datafox` —— DataFox 后端
  - 写自己的后端：见 [`docs/writing-a-trace-source-adapter.md`](docs/writing-a-trace-source-adapter.md)
- **打包**：ESM-only + d.ts；`react`/`react-dom` 作 peer；零 `@grafana/*` 依赖

## 用法

```tsx
import { TraceTimeline, adaptTrace } from '@datafox/trace-timeline';
import { otlpAdapter } from '@datafox/trace-timeline/adapters/otlp';

const trace = adaptTrace(otlpAdapter, otlpResponse); // 后端 wire 格式 → 派生 Trace
